...
//    echo $qry;exit;
    ...
    {
         $mykey = $row['comparison_type'].' , '. $row['province'];
         $this_time = $row['checked_at_time'];
          
        if($this_time <= '2018-04-10 16:00:00')
        {
            $timestamps[]=$this_time;
            
            
        }
        $disp_arr[$row['comparison_type']][$row['province']][$this_time]=$row['c'];
    }    
    ...
    foreach($disp_arr as $type => $type_data)
    {
        foreach($type_data as $pro => $pro_data)
        {
            $xmlstore .= ' <dataset seriesname="'.$type.','.$pro.'">';
            foreach($pro_data as $timestamp => $count)
            {   
