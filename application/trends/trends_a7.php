...
<div class="widget widget-tabs">    
    ...
    $qry_pre_calc = "  
        ...
        FROM
            pre_calculated_data
        WHERE
            pre_calculated_data.type = 'sdp_reporting_rate_district_wise'
    ...
    {
        $pre_calculated_data[$row_pre['dist_id']][$row_pre['key']]=$row_pre['value'];
    }
    ...
    { 
        ...
        $qry_1 = "  
            ...
            //echo $qry_1;exit;
                ...
                {
                    $total_sdps[$row_1['dist_id']]=$row_1['totalWH'];
      ...
      //inserting calculations to db
      foreach($rep_rate as $dist_id => $dist_data)
        {
          foreach($dist_data as $month => $r_rate)
            {
                $q_insert=" INSERT INTO  `pre_calculated_data` 
                          (  `level`, `type`, `prov_id`, `stk_id`, `dist_id`, `key_type`, `wh_id`, `key`, `value` ) 
                          VALUES 
                          (  '3', 'sdp_reporting_rate_district_wise', '1', NULL, '".$dist_id."', 'month', NULL, '".$month."', '".$r_rate."' );
                                   ";
...
//echo 'Reported:';print_r($reporting_wh_arr);exit;  
    ...
    {
        $xmlstore .= ' <dataset seriesname="'.$dist_id.'">';
        foreach($months_list as $k => $month)
        {   
